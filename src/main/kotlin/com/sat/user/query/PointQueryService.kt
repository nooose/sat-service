...
class PointQueryService(
    ...
    fun getTotalPoint(memberId: Long): Int {
        ...
        val totalPoint = points.score(RedisCacheName.RANKING.key, memberId)?.toInt()
        return totalPoint ?: jdslRepository.findOne {
            select(sum(Point::point))
                .from(entity(Point::class))
                .where(path(Point::memberId).equal(memberId))
        }?.toInt() ?: 0
    }

    fun getPoints(memberId: Long, cursorRequest: CursorRequest): PageCursor<List<MyPointQuery>> {
        val points = jdslRepository.findAll {
